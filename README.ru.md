...
- **Не высадит батарею.** Режим сам выключается ниже выбранного заряда, а по желанию —
  сразу при отключении от сети.
- **Даёт знать, что сработало.** Тихий тон при закрытии крышки и другой при открытии.
...
- **Чистый Swift**, без зависимостей, без фонового демона, без телеметрии.
...
> открывать. Начиная с macOS 15 Sequoia правая кнопка это больше не обходит: откройте
> `.pkg`, дайте себя заблокировать, затем зайдите в **Системные настройки →
> Конфиденциальность и безопасность** — там в разделе «Безопасность» появится кнопка
> **«Всё равно открыть»**. На macOS 14 и старше — правой кнопкой по файлу → **«Открыть»**
> → в диалоге ещё раз **«Открыть»**. В любом случае один раз.
...
При включённом режиме закрытие крышки играет приглушённый фарфоровый тон, открытие — тон
повыше: достаточно, чтобы убедиться, что Mac не уснул, ничего не открывая для проверки.
Выключается в меню, а при выключенном режиме не звучит вовсе — закрытие крышки тогда просто
означает сон.

Тоны синтезируются во время сборки скриптом
[`Tools/make-sounds.swift`](Tools/make-sounds.swift), так что аудиофайлов в репозитории нет.
На Mac без крышки нет ни звуков, ни самой настройки.

## Предохранитель по батарее

Запрет сна делает лёгкой ровно одну ошибку: включить режим, закрыть крышку, убрать ноутбук
в рюкзак — и он греется там, пока батарея не сядет в ноль. Поэтому режим сам выключается,
когда заряд опускается до выбранного порога (по умолчанию 20%), а при желании — сразу при
отключении от сети. Обе настройки лежат в меню, а уведомление сообщает, что именно сработало.

На Mac без встроенной батареи обе настройки просто не показываются — неработающий
переключатель хуже его отсутствия. Проверка идёт по типу источника питания, а не по
принципу «источник вообще есть»: подключённый к настольному Mac ИБП — тоже источник. А если
уровень заряда почему-то не читается, не срабатывает ничего: выключить режим по причине,
которую приложение не может назвать, хуже, чем не трогать его.

## Обновления

Раз в сутки приложение спрашивает у GitHub, нет ли релиза новее. Если есть — приходит
системное уведомление с кнопками «Скачать» и «Пропустить эту версию»: модальное окно
поверх вашей работы не появляется. А проверку, запущенную вручную из меню, приложение
отвечает окном — вы её ждёте. Проверку можно выключить в меню: тогда в сеть ничего не
уходит, пока вы сами не попросите.
...
**Чтобы удалить совсем**, выберите в меню **«Удалить SleepSwitch…»**. Сначала снимется
запрет сна — пока правило `sudo` ещё на месте и это можно сделать тихо, — затем удалятся
приложение, правило, запись об установке и настройки, после чего программа закроется.
Один пароль, одно подтверждение.

Из склонированного репозитория можно запустить [`uninstall.sh`](uninstall.sh) или сделать
то же руками:
